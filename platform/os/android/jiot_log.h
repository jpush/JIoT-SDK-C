...
#include "../../../include/jclient/jiot_client.h"
...
{\
    ...
    {\
        jiot_log_out(INFO_LOG_LEVL, "[I] %ld %s:%d %s()| "format"\n",(long int)time(NULL),__FILE__, __LINE__, __FUNCTION__,##__VA_ARGS__);\
    }\
}\
...
{\
    ...
    {\
        jiot_log_out(INFO_LOG_LEVL, "[D] %ld %s:%d %s()| "format"\n",(long int)time(NULL),__FILE__, __LINE__, __FUNCTION__,##__VA_ARGS__);\
    }\
}\
